...
- Codex: `.codex/skills`
- 共通Skills: `check`、`commit`、`openapi`、`issue`、`pr`、
...
`openapi`、`issue`、`pr`は、自然言語の依頼内容に応じた自動適用にも対応します。
...
GitHubへのIssue・PR作成やコミットなどの副作用を伴う操作は、明示的に依頼された場合
だけ実行します。SkillsとCommandsの使い分けや呼び出し方は
